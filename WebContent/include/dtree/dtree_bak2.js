...
dTree.prototype.node = function(node, nodeId) {
	...
	}	//以下为增加了checkbox的页面	if(this.config.check!=0){		var check_type = "checkbox";		var onclick_op = 'onclick="javascript:'+this.obj+'.cc('+nodeId+')"';		if(this.config.check==1) {			check_type = "checkbox";			onclick_op = 'onclick="javascript:'+this.obj+'.cc('+nodeId+')"';		} if(this.config.check==2) {			check_type = "radio";			onclick_op = "";		}		if(node.pid==-1)			str+= '<input type="'+check_type+'" style="display: none;" name="c' + this.obj + '" id="c'+ this.obj + nodeId + '" value="'+this.aNodes[nodeId].id+'" '+onclick_op+' />';		else {			if(node.isChecked) 				str+= '<input type="'+check_type+'" name="c' + this.obj + '" id="c'+ this.obj + nodeId + '" value="'+this.aNodes[nodeId].id+'" checked '+onclick_op+' />';			else str+= '<input type="'+check_type+'" name="c' + this.obj + '" id="c'+ this.obj + nodeId + '" value="'+this.aNodes[nodeId].id+'" '+onclick_op+' />';		}	} 
	if (node.url) {
