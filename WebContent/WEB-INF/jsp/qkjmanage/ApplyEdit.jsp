		...
		<div class="tab_warp main">
			<div class="dq_step">
				${path} <span class="opb lb op-area"> <s:if test="%{perWorkFlag=='null' || perWorkFlag==null}">
					...
					</s:else> 				<s:if test="apply.status>=40&&apply.status!=70">
						...
						<a id="outadd" href="javascript:;">生成出库单</a>
					</c:if> 
				</span>
			</div>
								...
								<s:elseif test="null != apply && 'mdy' == viewFlag">
	                                ...
	                                <!-- 类型为媒体投放：主管审核-大区。。。。 -->
										...
										<c:if test="${apply.status==9 && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK20',apply.apply_dept)==true}">
					                     ...
					                     <!-- 青稞酒销售公司流程：业务部经理（10）| 总经理 （20）| 营销中心副总（70）-->        
					                	<s:elseif test="apply.apply_dept.substring(0,1)==2&&apply.apply_dept.substring(0,3)!=220">
										...
										<s:submit id="apply_check70" name="apply_check70" cssClass="input-green" value=" 营销中心副总-审核通过" action="apply_check70" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="apply_check5" name="apply_check5" cssClass="input-red" value="审核不通过" action="apply_check5" onclick="return isOp('确定执行此操作?');" />
					                	   	...
					                	   	<s:elseif test="apply.apply_dept.substring(0,3)==220">
					                    ...
					                    <!-- 北京天佑德 流程（销售一部，销售二部，销管部）：业务部经理（10）| 总经理 （20）| 营销中心副总（70）-->   
										...
										<c:if test="${apply.status==26 && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK10',apply.apply_dept)==true}">
											<s:submit id="apply_check10" name="apply_check10" value="业务部经理审核通过" cssClass="input-green" action="apply_check10" onclick="return isOp('确定进行此操作?');" />
											<s:submit id="apply_check5" name="apply_check5" value="审核不通过" cssClass="input-red" action="apply_check5" onclick="return isOp('确定进行此操作?');" />
										</c:if>
					                	</s:if>
					                	<s:else> 
					                	<c:if test="${(apply.status==10 || apply.status==26) && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK10',apply.apply_dept)==true}">
											<s:submit id="apply_check10" name="apply_check10" value="业务部经理审核通过" cssClass="input-green" action="apply_check10" onclick="return isOp('确定进行此操作?');" />
											<s:submit id="apply_check5" name="apply_check5" value="审核不通过" cssClass="input-red" action="apply_check5" onclick="return isOp('确定进行此操作?');" />
										</c:if>
					                	</s:else>
					                	...
					                	<c:if test="${apply.status==20 && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK40',apply.apply_dept)==true}">
					                	  ...
					                	  <!-- 北京天佑德 流程（红酒）：副总（10）| 总经理 （10）-->   
					                	<s:elseif test="apply.apply_dept.substring(0,3)=='312'">
					                	  	<s:if test="apply.apply_dept.substring(0,5)=='31205'">
					                	  	 ...
					                	  	 <s:else>
					                	  	 	<s:if test="apply.apply_dept.substring(0,5)=='31203'">
					                	  		<c:if test="${apply.status==10 && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK9',null)==true}">
													<s:submit id="apply_check9" name="apply_check9" cssClass="input-green" value="办事处-审核通过" action="apply_check9" onclick="return isOp('确定执行此操作?');" />
													<s:submit id="apply_check5" name="apply_check5" cssClass="input-red" value="审核不通过" action="apply_check5" onclick="return isOp('确定执行此操作?');" />
												</c:if>
												
												<%-- <s:if test="apply.apply_dept.substring(0,5)=='31411' || apply.apply_dept.substring(0,7)=='3120320'">
							                	<c:if test="${apply.status==9 && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK26',apply.apply_dept)==true}">
													<s:submit  value="区域经理审核通过" cssClass="input-green" action="apply_check26" onclick="return isOp('确定进行此操作?');" />
													<s:submit id="apply_check5" name="apply_check5" value="审核不通过" cssClass="input-red" action="apply_check5" onclick="return isOp('确定进行此操作?');" />
												</c:if>
												<c:if test="${apply.status==26 && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK10',apply.apply_dept)==true}">
												...
												<c:if test="${(apply.status==20  || apply.status==50) && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK20',apply.apply_dept)==true}">
					                	  		...
					                	  		<s:elseif test="apply.apply_dept.substring(0,5)=='31204' || apply.apply_dept.substring(0,5)=='31202'">
					                	  ...
					                	  <s:else>
					                	<c:if test="${apply.status==10 && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK20',apply.apply_dept)==true}">
											<s:submit id="apply_check20" name="apply_check20" cssClass="input-green" value="运营总监-审核通过" action="apply_check20" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="apply_check5" name="apply_check5" cssClass="input-red" value="审核不通过" action="apply_check5" onclick="return isOp('确定执行此操作?');" />
										</c:if>
										<s:if test="apply.apply_dept=='312'">
					                	...
					                	<c:if test="${apply.status>=9 && apply.status<50 && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK40',apply.apply_dept)==true}">
											<s:submit id="apply_check40" name="apply_check40" cssClass="input-green" value="总经理-审核通过" action="apply_check40" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="apply_check5" name="apply_check5" cssClass="input-red" value="审核不通过" action="apply_check5" onclick="return isOp('确定执行此操作?');" />
										</c:if>
					                	</s:elseif>
					                	...
					                	<s:elseif test="apply.apply_dept.substring(0,1)==3">
					                	<s:if test="apply.apply_dept.substring(0,3)=='307'">
					                	  	<c:if test="${apply.status==10 && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK10',apply.apply_dept)==true}">
											<s:submit id="apply_check10" name="apply_check10" value="业务部经理审核通过" cssClass="input-green" action="apply_check10" onclick="return isOp('确定进行此操作?');" />
											<s:submit id="apply_check5" name="apply_check5" value="审核不通过" cssClass="input-red" action="apply_check5" onclick="return isOp('确定进行此操作?');" />
										</c:if>
										...
										<c:if test="${apply.status==20 && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK20',apply.apply_dept)==true}">
											<s:submit id="apply_check20" name="apply_check20" cssClass="input-green" value="总监-审核通过" action="apply_check20" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="apply_check5" name="apply_check5" cssClass="input-red" value="审核不通过" action="apply_check5" onclick="return isOp('确定执行此操作?');" />
										</c:if>
					                	  	</s:if>
					                	<s:else>
					                    ...
					                    <!-- 陕西销售公司 流程（销管部）：总经理 （20）| 营销中心副总（70）-->   
					                    	<s:if test="apply.apply_dept.substring(0,1)==5">
					                
					                    	 	
					                	<c:if test="${apply.status>=10 && apply.status<60 && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK40',apply.apply_dept)==true}">
										<s:submit id="apply_check40" name="apply_check40" cssClass="input-green" value="总经理-审核通过" action="apply_check40" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="apply_check60" name="apply_check60" cssClass="input-red" value="推送副总" action="apply_check60" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="apply_check5" name="apply_check5" cssClass="input-red" value="审核不通过" action="apply_check5" onclick="return isOp('确定执行此操作?');" />
										</c:if>
										<c:if test="${apply.status==70 && it:checkPermit('QKJ_QKJMANAGE_APPLY_CHECK70',apply.apply_dept)==true}">
					                	  ...
					                	  <!-- 陕西销售公司： 业务部经理（10） | 总经理 （20）| 营销中心副总（70）-->   

					                	<%-- <s:elseif test="apply.apply_dept.substring(0,1)==5">
									...
									<c:if test="${apply.status>=9 && it:checkPermit('QKJ_QKJMANAGE_APPLY_SPT',apply.apply_dept)==true}">
										<s:submit id="delete" name="delete" value="退回" action="apply_spt" cssClass="input-red" onclick="return isOp('确定执行此操作?将退回到未审核状态');" />
									</c:if>
									<s:if test="apply.status>=40&&apply.status!=70">
