                 ...
                 title="${it:formatDate(activeTime.jpass,'yyyy-MM-dd HH:mm:ss')}"
									...
									<div class="zhuangtai" title="${it:formatDate(active.sd_time,'yyyy-MM-dd HH:mm:ss')}">
										...
										<s:if test="active.sd_status==20">
											...
											<s:if test="active.sd_status==26">
											<font class="message_pass">区域经理已审</font>(${active.sd_user_name})</s:if>
										...
										<table width="100%" cellpadding="0" cellspacing="0" border="0" class="lb_jpin">
											...
											<s:iterator value="otherActiveProducts" status="sta">
												<tr>
													...
													<td><c:if test="${active.status==0 && it:checkPermit('QKJ_QKJMANAGE_ACTIVEPRODUCT_DEL',null)==true}">
										...
										<table width="100%" cellpadding="0" cellspacing="0" border="0" class="lb_jpin">
											...
											<s:iterator value="activePosms" status="sta">
												<tr>
													...
													<td>${note}</td>
													<td class="nw">￥${total_price}</td>
													<td class="nw"><c:if test="${active.status==0 && it:checkPermit('QKJ_QKJMANAGE_ACTIVEPOSM_DEL',null)==true}">
										...
										<table width="100%" cellpadding="0" cellspacing="0" border="0" class="lb_jpin">
											...
											<s:iterator value="activeMemcosts" status="sta">
												<tr>
													...
													<td>${note}</td>
													<td class="nw">￥${total_price}</td>
													<td class="nw"><c:if test="${active.status==0 && it:checkPermit('QKJ_QKJMANAGE_ACTIVEMEMCOST_DEL',null)==true}">
					...
					<s:if test="null != active && (active.status==1||active.status==2)">
							...
							<div class="label_hang">
								...
								<div class="label_rwbenx">
									...
									<s:if test="active.apply_dept.substring(0,3)=='220'">
										...
										<c:if test="${active.sd_status>=30 && active.smd_status>=30 && 10!=active.fd_status && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_FDSTATUS10',active.apply_dept)==true}">
												<s:submit cssClass="input-green" value="财务-审核通过" action="mdyActiveFDSTATUS10Two" onclick="return isOp('确定执行此操作?');" />
												<s:submit cssClass="input-red" value="审核不通过" action="mdyActiveFDStatus" onclick="return isOp('确定执行此操作?');" />
											</c:if>
											
										<c:if test="${20==active.sd_status  && 10!=active.fd_status && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SDSTATUS10',active.apply_dept)==true}">
											<s:if test="active.apply_dept.substring(0,5)!='22024'">
									...
									<%-- 	<s:if test="active.apply_dept.substring(0,5)=='31411' || active.apply_dept.substring(0,7)=='3120320'">
										<c:if test="${20==active.sd_status && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SDSTATUS26',active.apply_dept)==true}">
											<s:submit cssClass="input-green" value="区域经理-审核通过" action="mdyActiveSDStatus26" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="mdyActiveSDStatus5" name="mdyActiveSDStatus5" cssClass="input-red" value="审核不通过" action="mdyActiveSDStatus5" onclick="return isOp('确定执行此操作?');" />
										</c:if>
										
										<c:if test="${26==active.sd_status && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SDSTATUS10',active.apply_dept)==true}">
											<s:submit id="mdyActiveSDStatus10" name="mdyActiveSDStatus10" cssClass="input-green" value="业务部经理-审核通过" action="mdyActiveSDStatus10" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="mdyActiveSDStatus5" name="mdyActiveSDStatus5" cssClass="input-red" value="审核不通过" action="mdyActiveSDStatus5" onclick="return isOp('确定执行此操作?');" />
										</c:if>
										</s:if> --%>
										...
										<c:if test="${26==active.sd_status && active.smd_status==30 && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SDSTATUS10',active.apply_dept)==true}">
											<s:submit id="mdyActiveSDStatus10" name="mdyActiveSDStatus10" cssClass="input-green" value="业务部经理-审核通过" action="mdyActiveSDStatus10" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="mdyActiveSDStatus5" name="mdyActiveSDStatus5" cssClass="input-red" value="审核不通过" action="mdyActiveSDStatus5" onclick="return isOp('确定执行此操作?');" />
										</c:if>
										</s:if>
										<s:else>
										<c:if test="${(20==active.sd_status || 26==active.sd_status)  && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SDSTATUS10',active.apply_dept)==true}">
											<s:submit id="mdyActiveSDStatus10" name="mdyActiveSDStatus10" cssClass="input-green" value="业务部经理-审核通过" action="mdyActiveSDStatus10" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="mdyActiveSDStatus5" name="mdyActiveSDStatus5" cssClass="input-red" value="审核不通过" action="mdyActiveSDStatus5" onclick="return isOp('确定执行此操作?');" />
										</c:if>
										</s:else>
									...
									<c:if test="${10==active.sd_status && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SDSTATUS25',active.apply_dept)==true}">
										...
										<c:if test="${active.sd_status>=20 && active.sd_status<40 && 30==active.smd_status && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SDSTATUS30',active.apply_dept)==true}">
									...
									<s:if test="active.apply_dept.substring(0,1)==5">
										<c:if test="${active.sd_status>=20 && active.sd_status<60  &&   it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SDSTATUS50',active.apply_dept)==true}">
									...
									<s:if test="active.apply_dept.substring(0,4)!='2201'">
										...
										<s:else>
										<c:if test="${active.sd_status>20 && 30==active.smd_status && active.fd_status==10 && active.sd_status<60  &&   it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SDSTATUS50',active.apply_dept)==true}">
										<s:submit id="mdyActiveSDStatus50" name="mdyActiveSDStatus50" cssClass="input-green" value="总经理-审核通过" action="mdyActiveSDStatus50" onclick="return isOp('确定执行此操作?');" />
											<s:if test="active.apply_dept.substring(0,1)!=3">
												<s:submit cssClass="input-green" value="送审到副总" action="mdyActiveSDStatus30Two" onclick="return isOp('确定执行此操作?');" />
											</s:if>
						...
						<div class="label_main">
							<div class="label_hang">
								...
								<div class="label_rwbenx">
								<s:if test="null != active && active.status<2">
									
									<%-- <s:if test="active.apply_dept.substring(0,5)!='22024'">
										<c:if test="${10==active.smd_status && active.sd_status>=30 && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SMDSTATUS10',active.apply_dept)==true}">
									...
									<s:else> --%>
									<c:if test="${10==active.smd_status && active.sd_status>=20 && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SMDSTATUS10',active.apply_dept)==true}">aa
									<s:submit id="mdyActiveSMDStatus10" name="mdyActiveSMDStatus10" cssClass="input-green" value="销管经理-审核通过" action="mdyActiveSMDStatus10" onclick="return isOp('确定执行此操作?');" />
										<s:submit id="mdyActiveSMDStatus5" name="mdyActiveSMDStatus5" cssClass="input-red" value="审核不通过" action="mdyActiveSMDStatus5" onclick="return isOp('确定执行此操作?');" />
									</c:if>
									<%-- </s:else> --%>
									...
									<s:else>
									<c:if test="${30==active.smd_status && active.sd_status==40 && (active.fd_status==0 || active.fd_status==5) && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SMDSTATUS50',active.apply_dept)==true}">
										<s:submit id="mdyActiveSMDStatus50" name="mdyActiveSMDStatus50" cssClass="input-green" value="销管部经理-审核通过" action="mdyActiveSMDStatus50" onclick="return isOp('确定执行此操作?');" />
										<s:submit id="mdyActiveSMDStatus5" name="mdyActiveSMDStatus5" cssClass="input-red" value="审核不通过" action="mdyActiveSMDStatus5" onclick="return isOp('确定执行此操作?');" />
									</c:if>
									</s:else>
									
									<c:if test="${40==active.smd_status && (active.sd_status>20 || active.fd_status==10) && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SMDSTATUS40',active.apply_dept)==true}">
							...
							<div class="label_hang">
 								<s:if test="active.apply_dept.substring(0,1)==4">
									...
									<div class="label_rwbenx">
										...
										<c:if test="${active.sd_status>=70 && 20==active.fd_status && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_FDSTATUS30',active.apply_dept)==true}">
											<s:submit  cssClass="input-green" value="营销中心财务-审核通过" action="mdyActiveFDSTATUS30" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="mdyActiveFDStatus5" name="mdyActiveFDStatus5" cssClass="input-red" value="审核不通过" action="mdyActiveFDStatus" onclick="return isOp('确定执行此操作?');" />
										</c:if>
										<div class="statusInline">
											股份公司审核状态:
											...
											<s:if test="active.fd_status==10">
												<font class="message_pass">会计已审</font>(${active.fd_user_name} ${it:formatDate(active.fd_time,'yyyy-MM-dd HH:mm:ss')})</s:if>
												<s:if test="active.fd_status==20">
												<font class="message_pass">财务经理已审</font>(${active.fd_user_name} ${it:formatDate(active.fd_time,'yyyy-MM-dd HH:mm:ss')})</s:if>
												<s:if test="active.fd_status==30">
												<font class="message_pass">营销中心已审</font>(${active.fd_user_name} ${it:formatDate(active.fd_time,'yyyy-MM-dd HH:mm:ss')})</s:if>
										</div>
									</div>
								</s:if>
								<s:else>
								...
								<s:if test="%{(active.apply_dept.substring(0,2)=='21' || active.apply_dept.substring(0,2)=='30') && active.smd_status!=50}">
									...
									<c:if test="${active.sd_status>=40 && 10!=active.fd_status && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_FDSTATUS10',active.apply_dept)==true}">
											<s:submit id="mdyActiveFDSTATUS10" name="mdyActiveFDSTATUS10" cssClass="input-green" value="财务-审核通过" action="mdyActiveFDSTATUS10" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="mdyActiveFDStatus5" name="mdyActiveFDStatus5" cssClass="input-red" value="审核不通过" action="mdyActiveFDStatus" onclick="return isOp('确定执行此操作?');" />
										</c:if>
									</s:if>
