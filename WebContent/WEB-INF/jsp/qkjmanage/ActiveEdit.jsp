							...
							<div class="label_hang">
								...
								<div class="label_rwbenx">
									...
									<s:if test="active.apply_dept.substring(0,3)=='220'"><!-- 甘肃 隆东（办事处，大区，销冠，财务，总经理）其它（办事处，销冠，财务，总经理）-->
										...
										<s:else>
										<c:if test="${active.sd_status>=20 && active.smd_status>=30 && 10!=active.fd_status && it:checkPermit('QKJ_QKJMANAGE_ACTIVE_FDSTATUS10',active.apply_dept)==true}">
												<s:submit cssClass="input-green" value="财务-审核通过" action="mdyActiveFDSTATUS10Two" onclick="return isOp('确定执行此操作?');" />
												<s:submit cssClass="input-red" value="审核不通过" action="mdyActiveFDStatus" onclick="return isOp('确定执行此操作?');" />
										</c:if>
										</s:else>
									...
									<s:if test="active.apply_dept.substring(0,1)==5"><!-- 陕西总经理 -->
									...
									<s:if test="active.apply_dept.substring(0,3)=='220'"><!-- 甘肃总经理 -->
										<c:if test="${active.fd_status==10 && active.sd_status<60  &&   it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SDSTATUS50',active.apply_dept)==true}">
										<s:submit id="mdyActiveSDStatus50" name="mdyActiveSDStatus50" cssClass="input-green" value="总经理-审核通过" action="mdyActiveSDStatus50" onclick="return isOp('确定执行此操作?');" />
											<s:if test="active.apply_dept.substring(0,1)!=3">
												<s:submit cssClass="input-green" value="送审到副总" action="mdyActiveSDStatus30Two" onclick="return isOp('确定执行此操作?');" />
											</s:if>
									...
									<s:elseif test="active.apply_dept.substring(0,1)==3"><!-- 北京总经理 -->
										<c:if test="${active.sd_status==30 && 30==active.smd_status &&   it:checkPermit('QKJ_QKJMANAGE_ACTIVE_SDSTATUS50',active.apply_dept)==true}">
											<s:submit id="mdyActiveSDStatus50" name="mdyActiveSDStatus50" cssClass="input-green" value="总经理-审核通过" action="mdyActiveSDStatus50" onclick="return isOp('确定执行此操作?');" />
											<s:submit id="mdyActiveSDStatus5" name="mdyActiveSDStatus5" cssClass="input-red" value="审核不通过" action="mdyActiveSDStatus5" onclick="return isOp('确定执行此操作?');" />
										</c:if>
									</s:elseif>
