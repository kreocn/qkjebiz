	...
	<!-- 主表显示 -->
		<s:if test="null != outStock">
		  <tr>
			...
			<td class='secRow'><s:property value="outStock.ordernum" /><s:hidden name="outDetail.lading_id"  title="出库单号" /></td>
...
<tr>
		...
		手机:
		<s:textfield id="order_user_mobile" name="outStock.member_mobile" title="手机" controlName="手机" />
		姓名:
		...
		<td class='secRow' colspan="6">
		<s:textarea name="outStock.address" title="配送地点" cssStyle="width:80%;" rows="2" require="required" controlName="配送地点"></s:textarea>
		<br />
...
<s:elseif test="%{outStock.reason==3}"><!-- 报损出库，不审核，自动入库到报损库 -->
	<tr>
		...
		<td class='secRow' colspan="3">
		<s:textarea name="outStock.note" title="内容" cssStyle="width:80%;" rows="4" require="required" controlName="内容"></s:textarea>
	</tr>
</s:elseif>
