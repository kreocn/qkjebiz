...
<div id="result">
	...
	<div class="itabletitle">
		<span class="title1"><s:if test="null == order && 'add' == viewFlag">增加</s:if><s:elseif test="null != order && 'mdy' == viewFlag">修改</s:elseif>订单</span>
		<span class="extra1">
			<a href="<s:url action="order_list" namespace="/sysebiz"><s:param name="viewFlag">relist</s:param></s:url>" >订单列表</a>
		</span>
		<s:if test="order.status>=40">
		<span class="extra1">
			<a href="<s:url namespace="/sysebiz" action="order_view"><s:param name="order.uuid" value="order.uuid" /><s:param name="viewFlag">print</s:param></s:url>" target="_blank">订单打印</a>
		</span>
		</s:if>
	</div>
<s:form name="form1" action="order_add" namespace="/sysebiz" onsubmit="return validator(this);" method="post" theme="simple">
	...
	<table class="ilisttable" width="100%">
		...
		<tr>
		<td class='firstRow'><span style="color:red;">*</span> 购货人信息:</td>
		<td class='secRow' colspan="3">
			<table class="ilisttable" width="100%">
			<tr>
			<td class='secRow' style="font-weight: bold;padding-left: 10px;">
			会员号:
			<s:textfield id="order_user_id" name="order.user_id" title="会员号" require="required" dataLength="0,85" controlName="购货人" />
			手机:
			<s:textfield id="order_user_mobile" name="order.user_mobile" title="手机" controlName="购货人" />
			姓名:
			<s:textfield id="order_user_name" name="order.user_name" title="姓名" controlName="购货人" />
		...
		<td class='secRow' colspan="3">
			<s:select name="order.pay_type"
			list="#{0:'余额支付',1:'网上银行支付',2:'货到付款',3:'银行打款',4:'当面收取',5:'积分付款'}" />
		</td>
		</tr>
		<s:if test="null != order">
		<tr>
		...
		<td class='secRow'>
			...
			<s:if test="2==order.order_source">
				淘宝 (淘宝帐号:<s:property value="order.add_user" />)
			</s:if>
		...
		<!-- 
		<tr>
		<td class='firstRow'>配送方式:</td>
		<td class='secRow'></td>
		<td class='firstRow'>发货时间:</td>
		<td class='secRow'><s:date name="order.ship_time" format="yyyy-MM-dd HH:mm:ss" /></td>
		</tr>
		<tr>
		...
		<td class='secRow'>
			...
			<s:if test="1==order.order_source">
				代客下单 (下单人:<s:property value="order.add_user" />)
			</s:if>
			<s:if test="2==order.order_source">
				淘宝 (淘宝帐号:<s:property value="order.add_user" />)
			</s:if>
		...
		<!-- 
		<tr>
		<td class='firstRow'>发票类型:</td>
		<td class='secRow'></td>
		<td class='firstRow'>发票抬头:</td>
		<td class='secRow'></td>
		</tr>
		<tr>
		<td class='firstRow'>发票内容:</td>
		<td class='secRow'></td>
		...
		<td class='secRow' colspan="3">
		<s:textarea  name="order.note" title="备注" dataLength="0,400" controlName="备注" cssStyle="width:80%;"></s:textarea>
		</td>
		</tr>
		
		<tr><td class="titleRow" colspan="20">收货人信息 [<a id="moreaddresstext" href="javascript:;" onclick="showMoreAddress();">显示更多地址</a>]</td></tr>
		...
		<tr>
			...
			<td class='secRow'><s:textfield name="order.con_name" title="收货人姓名" dataLength="0,85" controlName="收货人姓名" require="required" /></td>
			<td class='firstRow'><span style="color:red;">*</span>联系电话:</td>
			<td class='secRow'><s:textfield name="order.con_mobile" title="手机/联系电话" dataLength="0,85" controlName="手机/联系电话" require="required" /></td>
		</tr>
		<tr>
			<td class='firstRow'><span style="color:red;">*</span>收货地区:</td>
			<td class='secRow'>
<s:select name="order.con_province" id="con_provinceid" title="省" list="#{}" require="required" controlName="所在地区"></s:select>
<s:select name="order.con_city" id="con_cityid" title="市" list="#{}"></s:select>
<s:select name="order.con_area" id="con_areaid" title="县" list="#{}"></s:select>
			<td class='firstRow'>邮编:</td>
			<td class='secRow'><s:textfield name="order.con_post" title="邮编" dataLength="0,85" controlName="邮编" /></td>
		</tr>
		<tr>
			<td class='firstRow'><span style="color:red;">*</span>街道地址:</td>
			<td class='secRow'><s:textarea name="order.con_street" title="街道地址" dataLength="0,21845" controlName="街道地址" require="required" cssStyle="width:80%;" /></td>
			<td class='firstRow'>收货人备注:</td>
			<td class='secRow'><s:textarea  name="order.con_note" title="收货人备注" dataLength="0,21845" controlName="收货人备注" cssStyle="width:80%;"></s:textarea></td>
		</tr>
		
		<tr>
		<td class="titleRow" colspan="20">
		商品信息
		<s:elseif test="(order.status>=10 && order.status<=36) && viewFlag=='mdy'">
...
<table class="ilisttable" id="table_goods_firstadd" width="100%">
<tr>
...
<td class='secRow'>
	...
	<span id="goods_unit_02"></span>
	<span id="goods_num_02" class="goods_num_show"></span>
</td>
...
<s:iterator value="orderGoodss" status="sta">
  <tr class="<s:if test="#sta.odd == true">oddStyle</s:if><s:else>evenStyle</s:else>">
	...
	<tr>
		...
		<td>￥<s:textfield name="order.total_price" title="订单总价" dataType="number" require="required" controlName="订单总价" /></td>
		<td></td>
	</tr>
</table>
<script type="text/javascript">setStyle("table_goods_list");</script>
		...
		<tr><td class="titleRow" colspan="20">订单操作信息</td></tr>
		<tr>
			<td class='firstRow'>操作备注:</td>
			<td class='secRow' colspan="3"><s:textarea name="op_message" title="操作备注" dataLength="0,21845" controlName="操作备注" cssStyle="width:80%;" /></td>
		</tr>
		<tr>
			<td class='firstRow'>当前可执行操作:</td>
			<td class='secRow' colspan="3"  class="buttonarea">
				<s:if test="null == order && 'add' == viewFlag">
					<s:if test="@org.iweb.sys.ContextHelper@checkPermit('QKJCJ_SYSEBIZ_ORDER_ADD')">
					<s:submit id="order_add" name="order_add" value="新增订单" action="order_add" />
					</s:if>
				...
				<s:if test="10==order.status && 'mdy' == viewFlag">
					...
					<s:submit id="save" name="save" value="保存订单" action="order_save" />
					</s:if>
					<s:if test="@org.iweb.sys.ContextHelper@checkPermit('QKJCJ_SYSEBIZ_ORDER_CHECK10')">
					<s:submit id="order_check10" name="order_check10" value="送交审核" action="order_check10" onclick="return isOp('是否送交审核?\n送审后将不能更改!');" />
					</s:if>
					<s:if test="@org.iweb.sys.ContextHelper@checkPermit('QKJCJ_SYSEBIZ_ORDER_CHECK0')">
					<s:submit id="order_check0" name="order_check0" value="订单作废" action="order_check0" onclick="return isOp('是否作废订单?\n作废操作不能取消!');" />
					</s:if>
				</s:if>
				<s:if test="15==order.status && 'mdy' == viewFlag">
					...
					<s:submit id="order_check15" name="order_check15" value="重新送审" action="order_check15" onclick="return isOp('是否重新送审?\n送审后将不能更改!');" />
					</s:if>
					<s:if test="@org.iweb.sys.ContextHelper@checkPermit('QKJCJ_SYSEBIZ_ORDER_CHECK0')">
					<s:submit id="order_check0" name="order_check0" value="订单作废" action="order_check0" onclick="return isOp('是否作废订单?\n作废操作不能取消!');" />
					</s:if>
				...
				<s:if test="20==order.status && 'mdy' == viewFlag">
					...
					<s:submit id="order_check19" name="order_check19" value="不通过" action="order_check19" onclick="return isOp('确定退回订单(审核不通过)?\n退回操作不能取消!');" />
					</s:if>
					<s:if test="@org.iweb.sys.ContextHelper@checkPermit('QKJCJ_SYSEBIZ_ORDER_CHECK20')">
					<s:submit id="order_check20" name="order_check20" value="审核通过" action="order_check20" onclick="return isOp('确定通过?\n通过后将直接进行商品配货!');" />
					</s:if>
					<s:if test="@org.iweb.sys.ContextHelper@checkPermit('QKJCJ_SYSEBIZ_ORDER_CHECK0')">
					<s:submit id="order_check0" name="order_check0" value="订单作废" action="order_check0" onclick="return isOp('是否作废订单?\n作废操作不能取消!');" />
					</s:if>
				...
				<s:if test="30==order.status && 'mdy' == viewFlag">
					...
					<s:submit id="order_check30" name="order_check30" value="配货完成" action="order_check30" onclick="return isOp('确定配货完成?\n配货完成操作不能取消!');" />
					</s:if>
					<s:if test="@org.iweb.sys.ContextHelper@checkPermit('QKJCJ_SYSEBIZ_ORDER_CHECK0')">
					<s:submit id="order_check0" name="order_check0" value="订单作废" action="order_check0" onclick="return isOp('是否作废订单?\n作废操作不能取消!');" />
					</s:if>
				...
				<s:if test="40==order.status && 'mdy' == viewFlag">
					...
					<s:submit id="order_check39" name="order_check39" value="重新配货" action="order_check39" onclick="return isOp('确定重新配货吗?\n重新配货操作不能取消!');" />
					</s:if>
					<s:if test="@org.iweb.sys.ContextHelper@checkPermit('QKJCJ_SYSEBIZ_ORDER_CHECK41')">
					<s:submit id="order_check41" name="order_check41" value="交易完成" action="order_check41" onclick="return isOp('确定交易完成?\n交易完成操作不能取消!\n交易完成之后,订单将不能进行任何操作!');" />
					</s:if>
					<s:if test="@org.iweb.sys.ContextHelper@checkPermit('QKJCJ_SYSEBIZ_ORDER_CHECK0')">
					<s:submit id="order_check0" name="order_check0" value="订单作废" action="order_check0" onclick="return isOp('是否作废订单?\n作废操作不能取消!');" />
					</s:if>
				...
				<s:if test="0==order.pay_status && 'mdy' == viewFlag">
					...
					<s:submit id="order_pay_status" name="order_pay_status" value="确认付款" action="order_pay_status" onclick="return isOp('是否确定钱已到账?\n此操作不能取消!');" />
					</s:if>
				</s:if>
...
<s:if test="'mdy' == viewFlag">
		<tr><td class="tablearea" colspan="20">
<table class="ilisttable" id="table_orderActns" width="100%">
	  <tr>
	  	<th>操作人</th>
		<th>操作时间</th>
	    <th>操作类型</th>
	  ...
	  <tr class="<s:if test="#sta.odd == true">oddStyle</s:if><s:else>evenStyle</s:else>">
	    ...
	    <td>
	    	...
	    	<s:elseif  test="'CHANGE_PAYSTATUS'==action_type">
	    		把付款状态改为:
	    		<s:if test="1==new_value">【已付款】</s:if>
	    	</s:elseif>
	    	<s:else>
	    		<s:property value="new_value" />
	    	</s:else>
	    </td>
