...
function Common_Ajax(result_div_id) {
	...
	this.config = { action_url : "/erp/common_ajax/json_ajax", action_data_type : "json",
		_beforeSend : function(xmlhttp) {
			...
			if (selfobj.config._checkData(data))
				selfobj.config._success(data, textStatus);
		}, _error : function(jqXHR, textStatus, errorThrown) {
			if (showmsg)
				$('#' + result_div_id + " #ajax_load_message").text("获取数据异常!" + textStatus + ":" + errorThrown);
		}, _complete : function() {
			if (showmsg) {
	...
	this.setParameter = function() {
		if (selfobj.keyCount != -1) { // post数据
			for ( var i = 0; i <= selfobj.keyCount; i++) {
				if (parameter == "")
					parameter = selfobj.keys[i] + '=' + selfobj.values[i];
				else
					parameter += "&" + selfobj.keys[i] + '=' + selfobj.values[i];
			}
		}
	...
	this.sendAjax = function() {
		this.setParameter();
		$.ajax({ type : 'post', url : this.config.action_url, data : parameter, beforeSend : this.config._beforeSend,
			dataType : this.config.action_data_type, success : this.config._success, error : this.config._error,
	...
	this.sendAjax2 = function() {
		this.setParameter();
		$.ajax({ type : 'post', url : this.config.action_url, data : parameter, beforeSend : this.config._beforeSend,
			dataType : this.config.action_data_type, success : this.config.__success, error : this.config._error,
			complete : this.config._complete });
	};
}