...
var addOrderGoodses = function(p_order_goods_id, p_goods_name, p_ware_id, p_ware_name, p_order_num,p_goods_id) {
	...
	thtml += '<span class="cellar_goods_name_value" data="' + p_goods_id + '">' + p_goods_name + '</span> | ';
...
var addCellarMember = function(p_uuid, p_order_goods_id, p_cellar_no, p_num, p_stock, p_camera, p_order_id, p_goods_id,
		p_goods_name, p_order_num, p_ware_id, p_ware_name) {
	var list_con_id = con_id + p_ware_id + p_goods_id;
	// 如果不存在,就创建,专门为列出用户藏酒和设定,配货时,使用addOrderGoodses来提前设定
	if($("#" + list_con_id).length==0) {
		thtml = "<div  id='" + list_con_id + "' class='cellar_list'>";
		thtml += '<div class="cellar_title">';
		...
		thtml += '<span class="cellar_goods_ware">所在仓库:</span>';
		thtml += '<span class="cellar_goods_ware_value" data="' + p_ware_id + '">' + p_ware_name + '</span> | ';
		thtml += '<span class="cellar_goods_pnum">拥有藏酒:</span>';
		thtml += '<span class="cellar_goods_pnum_value">0</span>';
		thtml += '</div>';
		thtml += '<ul></ul>';
		thtml += '</div>';
	...
	setPnum(p_ware_id + p_goods_id, p_cellar_no);
};
