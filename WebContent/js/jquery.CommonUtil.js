...
var CommonUtil = {
	...
	pickrow : function(table_id) {
		...
		$("#" + table_id + " tr[type='pickrow']").find(":checkbox").bind(
				"click", function() {
					if ($(this).is(":checked")) {
	...
	pickrowAll : function(table_id, sCheckboxName) {
		$("#" + table_id + " :checkbox[name='" + sCheckboxName + "']").click(
				function() {
	...
	checkAll : function(con_id, sCheckboxName) {
		$("#" + con_id + " :checkbox[name='" + sCheckboxName + "']").attr(
				"checked", "checked");
	},
	// 取消选择CheckBox
	uncheckAll : function(con_id, sCheckboxName) {
		$("#" + con_id + " :checkbox[name='" + sCheckboxName + "']")
				.removeAttr("checked");
	},
	...
	formatJSONDate : function(d, mode) {
		...
		if (d && typeof d == "object")
			return (d.year + 1900)
					+ "-"
					+ CommonUtil.pad(d.month + 1, 2)
					+ "-"
					+ CommonUtil.pad(d.date, 2)
					+ (mode == 0 ? " " + CommonUtil.pad(d.hours, 2) + ":"
							+ CommonUtil.pad(d.minutes, 2) + ":"
							+ CommonUtil.pad(d.seconds, 2) : "");
		else
			return "";
	},// 数字前面补0
	pad : function(num, n) {
	 ...
	 * 
	 * @param mode方式
	 *            0/无:只获得参数值,1:获得整个参数字符串 比如aa=aaa&bb=bbb,getUrlParamStr('bb')
	 *            mode=0或mode不存在 则返回bbb;mode=1则返回bb=bbb
	 */
